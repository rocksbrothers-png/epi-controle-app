...
| ~~Matrícula com unicidade **global**, não por empresa~~ — **resolvido (issue #168)** | Era `employees_employee_id_code_key`, UNIQUE sem `company_id`: o segundo tenant não conseguia usar a matrícula "1001" e a recusa revelava a existência de um colaborador de outro cliente. Substituído por `uq_employees_company_employee_code` — UNIQUE(`company_id`, `employee_id_code`) — alinhando o banco ao que a aplicação já fazia (`ensure_employee_identity_unique` e o índice de upsert desta importação sempre filtraram por `company_id`). Descartado `UNIQUE(legal_entity_id, employee_id_code)`: a coluna é nullable e no PostgreSQL NULLs são distintos, então a constraint não valeria para nenhuma linha sem CNPJ vinculado, além de quebrar a transferência entre CNPJs, que preserva a matrícula de propósito. Travado por `test_employee_registration_code_uniqueness_is_scoped_per_tenant` mais três testes de comportamento em `tests_postgres/` |
